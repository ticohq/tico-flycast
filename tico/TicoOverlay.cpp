...
// Include stb_image from core/deps
...
void TicoOverlay::LoadAccountData()
{
    ...
    const char *avatarPaths[] = {
        "sdmc:/tiicu/assets/avatar.jpg", // Correct path
        "sdmc:/tico/assets/avatar.jpg",  // Legacy path
...
void TicoOverlay::RenderSocialArea(ImDrawList *dl, ImVec2 displaySize)
{
    ...
    float sideMargin = 32.0f; // As per EmulatorScreen logic for consistency
    float topMargin = 32.0f;
    float barHeight = 50.0f; // Matching StatusBar height
...
void TicoOverlay::RenderTitleCard(ImDrawList *dl, ImVec2 displaySize)
{
    ...
    const float TITLE_HEIGHT = 72.0f;
    const float AVAILABLE_TOP_SPACE = 110.0f;
    ...
    float scale = (displaySize.y > 1080.0f) ? 1.5f : 1.0f; // Typical scaling approach for overlay
...
void TicoOverlay::RenderSettingsMenu(ImDrawList *dl, ImVec2 displaySize)
{
    ...
    for (int i = 0; i < numItems; i++)
    {
        ...
        float valueX = itemMax.x - valueSize.x - 40.0f;
        ...
        if (isSelected)
        {
            ...
            // Left Arrow
            float leftArrowX = valueX - arrowSize - 12.0f;
            ...
            // Right Arrow
            float rightArrowX = valueX + valueSize.x + 12.0f;
...
void TicoOverlay::RenderHelpersBar(ImDrawList *dl, ImVec2 displaySize)
{
    ...
    for (size_t i = 0; i < helpers.size(); i++)
    {
        ...
        totalWidth += BUTTON_SIZE + 8.0f + textW;
        if (i < helpers.size() - 1)
            totalWidth += ITEM_SPACING;
    }

    // Animate from right
    float startOffset = 400.0f;
...
void TicoOverlay::RenderStatusBar(ImDrawList *dl, ImVec2 displaySize)
{
    ...
    const float TOP_MARGIN = 32.0f;
    const float SIDE_MARGIN = 32.0f;
    const float ITEM_SPACING = 20.0f;
    ...
    if (showBattery)
    {
        totalWidth += ITEM_SPACING + 34.0f; // Battery icon width + space
    }
    ...
    if (showBattery)
    {
        ...
        float bodyWidth = 32.0f;
        float bodyHeight = 20.0f;
        float tipWidth = 4.0f;
        float tipHeight = 10.0f;
        ...
        if (m_isCharging)
        {
            ...
            if (m_boltTexture != 0)
            {
                ...
                ImVec2 iconPos = ImVec2(tipMax.x + 6.0f, batteryPos.y + (bodyHeight - iconH) * 0.5f);
