/// @file TicoMain.cpp
/// @brief Entry point for tico-integrated Flycast NRO
...
// #ifdef __SWITCH__
// // Custom Exception Handler for Flycast JIT (forward declared from libnx_vmem.cpp)
// extern "C" {
//     // Flycast's handler
//     void flycast_nx_exception_handler(ThreadExceptionDump *ctx);

//     // Override libnx's weak symbol
//     void __libnx_exception_handler(ThreadExceptionDump *ctx) {
//         flycast_nx_exception_handler(ctx);
//     }
// }
// #endif
...
bool InitWindow()
{
    ...
    g_window = SDL_CreateWindow("Flycast",
...
void HandleInput()
{
    ...
    // Tico Overlay Mode
    // Let overlay consume input if visible
    if (g_overlay && g_overlay->HandleInput(controller))
    {
...
#ifdef __SWITCH__
            const char *nextNro = "sdmc:/switch/tico.nro";

            // Check if nextNro exists
            struct stat buffer;
            if (stat(nextNro, &buffer) == 0)
            {
                // Build args as space-separated string (per libnx envSetNextLoad docs)
                // Format: "nro_path --resume"
                char args[512];
                snprintf(args, sizeof(args), "%s --resume", nextNro);

                envSetNextLoad(nextNro, args);
                LOG_INFO("HOME", "Chainloading back to %s with args: %s", nextNro, args);
            }
            else
            {
                LOG_WARN("HOME", "Chainload target %s not found! Exiting normally.", nextNro);
            }
    ...
    if (g_core && controller)
    {
        g_core->ClearInputs();

        // Map controller to libretro
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_A,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_A));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_B,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_B));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_X,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_X));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_Y,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_Y));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_START,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_START));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_SELECT,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_BACK));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_UP,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_DPAD_UP));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_DOWN,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_DPAD_DOWN));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_LEFT,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_DPAD_LEFT));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_RIGHT,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_DPAD_RIGHT));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_L,
                              SDL_GameControllerGetButton(controller, SDL_CONTROLLER_BUTTON_LEFTSHOULDER));
        g_core->SetInputState(0, RETRO_DEVICE_ID_JOYPAD_R,
...
{
    LOG_INFO("HOME", "Starting in Tico Mode (default).");
    ...
    // 2. Socket initialization (required for thread resources)
    ...
    LOG_INFO("HOME", "Flycast starting...");
